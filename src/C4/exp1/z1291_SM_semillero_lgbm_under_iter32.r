...
PARAM <- list()
PARAM$experimento <- "ZZ9430_iter32"
...
setorder(tb_log, iteracion_bayesiana)
tb_log_32 <- tb_log[1:32]
setorder(tb_log_32, -ganancia)
...
for( ksemilla in ksemillas[PARAM$indice_inicio_semilla:PARAM$indice_fin_semilla] )
{
  ...
  parametros <- as.list(copy(tb_log_32[PARAM$modelo]))
